...
(function() {
  ...
  dljs.crushingDraw = function(html){ // Draw html content
    this.el.innerHTML = html; //previous_html_lines + new_content;
  };
  ...
  dljs.trace = function(arr){
    ...
    dljs.clearBoard = function( boardID ) {
    ...
    dljs.paintArray = function( arrOfLines, overwrite ){ // [ [ x1, y1, x2, y2, w, col ], ... ]
        //console.log("arrOfLines:", arrOfLines );
        ...
        if ( overwrite ) this.crushingDraw( str ); else this.draw( str ); 
    };
  ...
  dljs.getLineString = function (idString,x1,y1,x2,y2,weight,color,opacity, roundBorder, longSombra, colSombra){
      ...
      var ang = angRadians * this.CONST_180_BY_PI; //* 180 / Math.PI;
  ...
  dljs.getFastLineString = function (x1,y1,x2,y2,weight,color){
      ...
      var cathetus1 = x2-x1,
          ...
          ang = angRadians * this.CONST_180_BY_PI;
